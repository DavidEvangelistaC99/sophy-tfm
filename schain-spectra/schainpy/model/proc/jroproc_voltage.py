...
class Decoder(Operation):
    ...
    def __convolutionInTime(self, data, code_1, code_2, DC_1, H0, RMIX):
        ...
        # code_short = numpy.array(dc_2, dtype = numpy.complex64)

        # print(type(dc_2[0]))

        # t = [i for i in range(len(dc_2))]
        # plt.plot(t, numpy.real(dc_2))
        # plt.plot(t, numpy.imag(dc_2))
        # plt.show()
        ...
        for i in range(self.__nChannels):
            ...
            corr_1 = numpy.correlate(data[i,:], code_1, mode='full')[self.nBaud-1:]
            corr_2 = numpy.correlate(data[i,:], code_2, mode='full')[self.nBaud-1:]

            d = -int(DC_1*len(data[i,:])/100)
            # Adjust displacement, review

            corr_2 = numpy.roll(corr_2, d)

            range_km = 60
            r = int((RMIX + -(H0))*len(data[i,:])/range_km)
    ...
    def run(self, dataOut, code=None, nCode=None, nBaud=None, mode = 0, osamp=None, times=None, code_1=None, code_2=None, DC_1=None, H0=None, RMIX=None):
        ...
        if dataOut.flagDataAsBlock:
            ...
            """
            if mode == 0:
                # Función que estamos usando
                datadec = self.__convolutionInTime(dataOut.data, code_1, code_2, DC_1, H0, RMIX)
